...
export default function SuperAdminPage() {
  ...
  useEffect(() => {
    // Check Super Admin Authentication Session
    const userSession = typeof window !== 'undefined' ? localStorage.getItem('tuturnito_current_user') : null;
    if (!userSession) {
  ...
  if (!isAuthorized) {
    return (
      <div className="min-h-screen bg-slate-950 text-white flex flex-col items-center justify-center p-6 space-y-4">
        <div className="w-14 h-14 rounded-3xl bg-gradient-to-tr from-rose-600 to-indigo-600 flex items-center justify-center text-white shadow-2xl shadow-rose-500/20 animate-pulse">
          <ShieldAlert size={28} />
        </div>
        <div className="text-center space-y-1">
          <h3 className="text-base font-bold text-white tracking-tight">Verificando Credenciales de Super Admin...</h3>
          <p className="text-xs text-slate-400">tuturnito<span className="text-rose-400">.master</span></p>
        </div>
      </div>
    );
  }
      ...
      <header className="border-b border-slate-800 bg-slate-900/80 backdrop-blur-md sticky top-0 z-40 px-4 lg:px-8 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="p-2 text-slate-400 hover:text-rose-400 hover:bg-rose-500/10 rounded-lg transition border border-slate-800"
          >
            <LogOut size={18} />
          </button>
        </div>
